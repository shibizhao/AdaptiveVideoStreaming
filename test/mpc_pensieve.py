# Author: Hongzi Mao
# Website: https://github.com/hongzimao/pensieve
# Description: A robust MPC implementation of adaptive bitrate selection to realize the maximum QoE
...
def main():
    ...
    while True:  # serve video forever
        ...
        tmp_str = str(time_stamp / M_IN_K) + '\t' + \
                       ...
                       str(delay) + '\t' + \
                       str(reward) + '\n'
        log_file.write(tmp_str.encode('utf-8'))
        log_file.flush()
        ...
        # ================== MPC =========================
        curr_error = 0 
        # defualt assumes that this is the first request so error is 0 since we have never predicted bandwidth
       ...
       # past_bandwidth_ests.append(future_bandwidth)
        ...
        if end_of_video:
            ...
            log_path = LOG_FILE + '_pensieve_' + all_file_names[net_env.trace_idx]
            log_file = open(log_path, 'wb')
