...
#   An implementation of FastMPC method using a vectorization Look-up Table.
# Update:
...
def initialLookUpTable():
    enumFP = open("enum_vec.txt",'r',encoding='utf-8',errors='ignored')
    while True:
        ...
        if content:
            ...
            for i in contentArray:
                if i[0] != '\n':
                    lookupTable.append(int(i))
            assert len(lookupTable) == 15000
        else:
            break
    enumFP.close()
